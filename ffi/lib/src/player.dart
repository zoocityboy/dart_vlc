...
class VideoDimensions extends Equatable {
...
class Player {
  ...
  Player({
    ...
    final pointers = this.commandlineArguments.map<Pointer<Utf8>>((e) {
  ...
  void open(MediaSource source, {bool autoStart = true}) {
    ...
    if (source is Playlist) {
      final medias = <String>[];
      for (final media in source.medias) {
        medias.addAll(
          <String>[
      ...
      // Parse [commandlineArguments] & convert to `char*[]`.
      final pointers = medias.map<Pointer<Utf8>>((e) {
        return e.toNativeUtf8();
      }).toList();
      // ignore: omit_local_variable_types
  ...
  void add(Media source) {
    final sourceMediaTypeCStr = source.mediaType.name.toNativeUtf8();
    final sourceResourceCStr = source.resource.toNativeUtf8();
    PlayerFFI.add(
      id,
      sourceMediaTypeCStr,
      sourceResourceCStr,
    ...
    calloc
      ..free(sourceMediaTypeCStr)
      ..free(sourceResourceCStr);
  }
  ...
  int get audioTrackCount {
    final count = PlayerFFI.getAudioTrackCount(id);
